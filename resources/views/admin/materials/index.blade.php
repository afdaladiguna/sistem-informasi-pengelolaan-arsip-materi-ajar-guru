            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <div class="flex justify-between items-center mb-4">
                        ...
                        <a href="{{ route('materials.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-500">Unggah Materi Baru</a>
                    </div>
                    @if (session('success'))
                        ...
                        <table class="w-full text-sm text-left text-gray-500">
                            <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-6 py-3">Diunggah Pada</th>
                                    <th class="px-6 py-3">Diperbarui Pada</th>
                            ...
                            <tbody>
                                ...
                                <tr class="bg-white border-b hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4">{{ $material->user->nama_lengkap }}</td>
                                    <td class="px-6 py-4">{{ $material->created_at->format('d/m/Y H:i') }}</td>
                                    <td class="px-6 py-4">{{ $material->updated_at->format('d/m/Y H:i') }}</td>
                                    <td class="px-6 py-4">
                                        <a href="{{ Storage::url($material->file_path) }}" target="_blank" class="font-medium text-green-600 hover:underline mr-3">Lihat</a>
                                        <a href="{{ route('materials.edit', $material) }}" class="font-medium text-blue-600 hover:underline mr-3">Edit</a>
                                ...
                                @empty
                                <tr>
                                    <td colspan="7" class="px-6 py-4 text-center">Belum ada materi ajar yang diunggah.</td>
                                </tr>
                                @endforelse
                            </tbody>
                        </table>
